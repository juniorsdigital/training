...
async function calSaveModal() {
  ...
  const existingRow = calModalState.dayId
    ? (activePlan.days || []).find((d) => String(d.id) === String(calModalState.dayId))
    : null;
  const sessionSlot = existingRow
    ? Number.isInteger(Number(existingRow.session_slot))
      ? Number(existingRow.session_slot)
      : 0
    : calNextSessionSlot(calModalState.isoDate, activePlan);
  const payload = {
...
function calNextSessionSlot(isoDate, plan) {
  const indices = calDateToPlanIndices(isoDate, plan);
  let maxSlot = -1;
  (plan?.days || []).forEach((d) => {
    if (Number(d.week_index) !== Number(indices.week_index)) return;
    if (Number(d.day_index) !== Number(indices.day_index)) return;
    const s = Number(d.session_slot);
    if (Number.isInteger(s) && s >= 0) maxSlot = Math.max(maxSlot, s);
  });
  return maxSlot + 1;
}

function calSwapTargetOnDate(isoDate, fromRow) {
  const fromSlot = Number.isInteger(Number(fromRow.session_slot)) ? Number(fromRow.session_slot) : 0;
  const rows = (activePlan?.days || []).filter((d) => String(d.day_date).slice(0, 10) === isoDate);
  const sameSlot = rows.find((d) => (Number.isInteger(Number(d.session_slot)) ? Number(d.session_slot) : 0) === fromSlot);
  if (sameSlot && String(sameSlot.session_type || 'rest').toLowerCase() !== 'rest') {
    return sameSlot;
  }
  return null;
}
...
async function calDropSession(fromDayId, targetIso) {
  ...
  const targetWorkout = calSwapTargetOnDate(targetIso, fromRow);
