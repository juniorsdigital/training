...
function calUpdateModalPreview() {
  ...
  const goals = lib.buildGoalsFromWorkoutForm({
    ...
    watts: wattsEl?.value ? Number(wattsEl.value) : null
  });
  const est = lib.estimateProjectedTss({
    sessionType: st,
    durationMin: Number(durEl?.value),
  ...
  preview.textContent =
    (est.tss != null ? '~' + est.tss + ' TSS' : '— TSS') + (ifStr ? ' · ' + ifStr : '') + (durStr ? ' · ' + durStr : '');
}
...
function calOpenModal(mode, isoDate, dayRow) {
  ...
  if (wattsEl) wattsEl.value = lib?.goalValue(dayRow?.goals, 'goal_wattage') || lib?.parseWatts(dayRow?.target_watts_label) || '';
...
async function calSaveModal() {
  ...
  const payload = {
    ...
    target_watts_label: wattsEl?.value ? String(wattsEl.value) + 'W' : null,
...
function calInitInteractions() {
  ...
  ['calModalType', 'calModalDur', 'calModalWatts', 'calModalManualTss', 'calModalTss'].forEach((id) => {
