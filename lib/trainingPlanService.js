...
async function movePlanSession(planId, fromDayId, toWeekIndex, toDayIndex, options = {}) {
  ...
  const targetExisting = await findPlanDaySlot(planId, toWeekIndex, toDayIndex, fromDay.session_slot || 0);
  const phaseRow =
  ...
  const toDay =
    ...
    (await findOrCreatePlanDaySlot(planId, {
      ...
      day_date: dayDateFromPlanIndices(plan, toWeekIndex, toDayIndex),
      session_slot: fromDay.session_slot || 0,
