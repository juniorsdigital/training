...
const { applyProjectedTssGoals, DEFAULT_FTP } = require('./projectedTss.js');
const { transferSession, clearToRest, cloneDaySnapshot } = require('./trainingPlanAiOps.js');
...
function mergePlanDayRows(rows) {
  ...
  sorted.forEach((d) => {
    ...
    types.push(String(d.session_type || 'rest').toLowerCase());
...
function dayDateFromPlanIndices(plan, weekIndex, dayIndex) {
  const start = new Date(`${asIsoDate(plan.start_date)}T00:00:00`);
  const days = Array.isArray(plan?.days) ? plan.days : [];
  const usesOneBased = days.some((d) => Number(d.week_index) >= 1);
  const weekOffset = usesOneBased ? Number(weekIndex) - 1 : Number(weekIndex);
  const d = new Date(start);
  d.setDate(d.getDate() + weekOffset * 7 + Number(dayIndex));
...
async function upsertPlanDay(planId, day, options = {}) {
  ...
  const goalsWithTss = applyProjectedTssGoals(row, day?.goals || [], {
    ftp: safeFtp,
    manualOverride: Boolean(options.manualOverride),
    enforceTss: Boolean(options.enforceTss)
  });
  const goals = await replaceDayGoals(row.id, goalsWithTss);
  return { ...row, goals };
}

async function deletePlanDay(planDayId) {
  const id = String(planDayId || '').trim();
  if (!id) throw new Error('planDayId is required.');
  await requestJson(`/training_plan_days?id=eq.${encodeURIComponent(id)}`, {
    method: 'DELETE',
    headers: supabaseHeaders()
  });
  return { ok: true, id };
...
async function findPlanDaySlot(planId, weekIndex, dayIndex, sessionSlot = 0) {
  const slot = Number(sessionSlot) || 0;
  const rows = await requestJson(
    `/training_plan_days?plan_id=eq.${encodeURIComponent(planId)}&week_index=eq.${encodeURIComponent(
      weekIndex
    )}&day_index=eq.${encodeURIComponent(dayIndex)}&session_slot=eq.${encodeURIComponent(slot)}&select=*`
  );
  return Array.isArray(rows) && rows.length ? rows[0] : null;
}

async function findOrCreatePlanDaySlot(planId, { week_index, day_index, day_date, session_slot = 0, phase_label, phase_code }) {
  const existing = await findPlanDaySlot(planId, week_index, day_index, session_slot);
  if (existing) return existing;
  const plan = await getPlanById(planId);
  if (!plan) throw new Error('Plan not found.');
  const resolvedDate = isIsoDate(day_date)
    ? asIsoDate(day_date)
    : dayDateFromPlanIndices(plan, week_index, day_index);
  return upsertPlanDay(
    planId,
    {
      week_index,
      day_index,
...
async function movePlanSession(planId, fromDayId, toWeekIndex, toDayIndex, options = {}) {
  const plan = await getPlanById(planId);
  if (!plan) throw new Error('Plan not found.');
  const fromDay = (plan.days || []).find((d) => String(d.id) === String(fromDayId));
  if (!fromDay) throw new Error('Source workout not found.');

  const sessionSlot = Number.isInteger(Number(fromDay.session_slot)) ? Number(fromDay.session_slot) : 0;
  const toDayDate = isIsoDate(options.toDayDate)
    ? asIsoDate(options.toDayDate)
    : dayDateFromPlanIndices(plan, toWeekIndex, toDayIndex);

  const targetExisting = await findPlanDaySlot(planId, toWeekIndex, toDayIndex, sessionSlot);
  const phaseRow =
    (plan.days || []).find((d) => Number(d.week_index) === Number(toWeekIndex)) || fromDay;
  const toDay =
    targetExisting ||
    (await findOrCreatePlanDaySlot(planId, {
      week_index: toWeekIndex,
      day_index: toDayIndex,
      day_date: toDayDate,
      session_slot: sessionSlot,
      phase_label: phaseRow.phase_label,
      phase_code: phaseRow.phase_code
    }));

  if (String(fromDay.id) === String(toDay.id)) {
    throw new Error('Source and destination are the same day.');
  }
  ...
  const ftp = Number(options.ftp) > 0 ? Number(options.ftp) : DEFAULT_FTP;
  const savedTo = await upsertPlanDay(planId, { ...toSnap, id: toDay.id }, { ftp });
  const savedFrom = await upsertPlanDay(planId, { ...fromSnap, id: fromDay.id }, { ftp });
  return { from: savedFrom, to: savedTo };
}

async function swapPlanSessions(planId, dayAId, dayBId, options = {}) {
  const plan = await getPlanById(planId);
  if (!plan) throw new Error('Plan not found.');
  const dayA = (plan.days || []).find((d) => String(d.id) === String(dayAId));
  const dayB = (plan.days || []).find((d) => String(d.id) === String(dayBId));
  if (!dayA || !dayB) throw new Error('One or both workouts not found.');
  if (String(dayA.id) === String(dayB.id)) {
    throw new Error('swap requires two distinct days.');
  }
  ...
  const ftp = Number(options.ftp) > 0 ? Number(options.ftp) : DEFAULT_FTP;
  const savedA = await upsertPlanDay(planId, { ...snapA, id: dayA.id }, { ftp });
  const savedB = await upsertPlanDay(planId, { ...snapB, id: dayB.id }, { ftp });
  return { day_a: savedA, day_b: savedB };
}
...
function mapPlanToLegacyWeeks(plan) {
  ...
  groups.forEach((rows) => {
    const first = rows[0];
    const weekNum = Number(first.week_index);
    ...
    documentation: {
      ...
      goal_types: {
        ...
        examples: [
          ...
          { goal_type: 'goal_duration_min', target_value: 90, unit: 'min', notes: 'Workout duration for projected TSS' },
          { goal_type: 'goal_tss', target_value: 65, unit: 'TSS', notes: 'source:auto or source:manual' },
          { goal_type: 'goal_if', target_value: 0.88, unit: null, notes: 'Optional explicit intensity factor' },
...
module.exports = {
  ...
  upsertPlanDay,
  deletePlanDay,
  findOrCreatePlanDaySlot,
  movePlanSession,
  swapPlanSessions,
  dayDateFromPlanIndices,
  replaceDayGoals,
  replacePlanDays,
