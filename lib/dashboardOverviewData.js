...
const { getCanonicalPlan, mapPlanToLegacyWeeks } = require('./trainingPlanService.js');
...
function planWeekIndexFromStart(startIso, localDateStr) {
...
function planWeekIsoRange(planStartIso, localDateStr) {
  const idx = planWeekIndexFromStart(planStartIso, localDateStr);
  if (idx < 0) return null;
  const base = new Date(`${planStartIso}T12:00:00`);
  ...
  return { start: fmt(start), end: fmt(end), weekIndex: idx };
}
...
async function buildDashboardOverviewPayload(localDate, options = {}) {
  ...
  activities.forEach((a) => {
    if (activityLocalDay(a) === localDate) {
      const raw = a.icu_training_load ?? a.training_load;
      const n = Number(raw);
      if (!Number.isNaN(n) && n >= 0) tssToday += n;
    }
  });

  let tssWeekActual = null;
  let tssWeekGoal = null;
  try {
    const plan = await getCanonicalPlan();
    if (plan?.start_date) {
      const weeksMapped = mapPlanToLegacyWeeks(plan);
      const wIdx = planWeekIndexFromStart(plan.start_date, localDate);
      if (wIdx >= 0 && wIdx < weeksMapped.length && weeksMapped[wIdx].tss != null) {
        tssWeekGoal = weeksMapped[wIdx].tss;
      }
      const rng = planWeekIsoRange(plan.start_date, localDate);
      if (rng) {
        let sum = 0;
        activityHistory.forEach((a) => {
          const d = String(a.startLocal || '').slice(0, 10);
          if (!d || d < rng.start || d > rng.end) return;
          if (d > localDate) return;
          const load = a.icuTrainingLoad;
          const n = Number(load);
