...
function parsePlanCsv(text) {
  // #region agent log
  try {
    const _t = typeof text;
    const _str = String(text || '');
    fetch('http://127.0.0.1:7393/ingest/08dac9f5-b509-4991-86ef-01bcfd09de75',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'230f27'},body:JSON.stringify({sessionId:'230f27',hypothesisId:'A,B,C,D',location:'lib/trainingPlanCsv.js:124',message:'parsePlanCsv invoked',data:{textType:_t,strLen:_str.length,preview:_str.slice(0,200),hasNewline:_str.includes('\n'),hasCarriageReturn:_str.includes('\r')},timestamp:Date.now()})}).catch(()=>{});
  } catch (_e) {}
  // #endregion
  const rows = parseCsv(String(text || ''));
  // #region agent log
  try {
    fetch('http://127.0.0.1:7393/ingest/08dac9f5-b509-4991-86ef-01bcfd09de75',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'230f27'},body:JSON.stringify({sessionId:'230f27',hypothesisId:'D',location:'lib/trainingPlanCsv.js:125',message:'parsePlanCsv parsed rows count',data:{rowsLen:rows.length,firstRowLen:rows[0]?.length||0,firstRowPreview:JSON.stringify(rows[0]||null).slice(0,200)},timestamp:Date.now()})}).catch(()=>{});
  } catch (_e) {}
  // #endregion
